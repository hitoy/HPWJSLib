...
attachDragHandle();attachWheelHandle();};__init();this.play=function(){if(slidernum==0||slidernum==slidernuminview)return false;var start=function(){autoplayid=setTimeout(function(){slide(currentindex+step);start();},delay);};var stop=function(){clearTimeout(autoplayid);};['mouseenter','touchstart'].forEach(function(ev){carouselwrap.parentNode.addEventListener(ev,function(){stop();});},{passive:true});['mouseleave','touchend'].forEach(function(ev){carouselwrap.parentNode.addEventListener(ev,function(){start();});},{passive:true});start();}
