...
var order=1;carouselscroll.childNodes.forEach(function(c,i){if(i>=index&&i<index+slidercountinview){c.classList.add(carouselscrollactiveclass);c.setAttribute('data-order',order);order++;}else{c.classList.remove(carouselscrollactiveclass);c.removeAttribute('data-order');}});in_transition=false;if(loop){if(index<step){move(index+slidercount,0);}
else if(index>=slidercount+step){move(index-slidercount,0);}}},duration);}
...
if(indicator){indicator.addEventListener('click',function(e){stop();var target=e.target||window.event.srcElement;if(disabled()||target.nodeName!='SPAN')return false;var index=(Array.from(indicator.children).indexOf(target));if(loop){index=index*step+step;}else{index=index*step;}
slide(index);});}
