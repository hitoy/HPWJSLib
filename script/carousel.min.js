 ...
 */
(function(w){'use strict';var version='0.0.6';var readyState=w.document.readyState;var carousels;if(readyState==="interactive"||readyState==="complete"){init();}else{w.document.addEventListener("DOMContentLoaded",init);}
...
index=0;else if(index>=slidercount-slidercountinview)
index=slidercount-slidercountinview;}
if(indicator)
carousel_indicator(index);move(index,duration);setTimeout(function(){if(!loop&&currentindex>=slidercount-slidercountinview){disable_page_button(nextbutton);}
...
slide(currentindex+step);else if(movedis>basedis/3)
slide(currentindex-step);else
slide(currentindex);carouselwrap.style.touchAction='none';},{passive:true});if(mousewheel){var sliderslength=carouselscroll.children.length;carouselscroll.addEventListener('wheel',function(ev){_this.stop();if(event.deltaY>0){if(!loop&&currentindex==sliderslength-1&&!disabled())return false;if(!disabled()){slide(currentindex+step);}}else if(event.deltaY<0){if(!loop&&currentindex==0&&!disabled())return false;if(!disabled()){slide(currentindex-step);}}
